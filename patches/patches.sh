...
for i in "${patch_files[@]}"; do
    ...
    security/selinux/hooks.c)
        ...
        sed -i '/int nosuid = (bprm->file->f_path.mnt->mnt_flags & MNT_NOSUID);/a\int rc, error;\nu32 seclen;' security/selinux/hooks.c
        sed -i '/int nosuid = (bprm->file->f_path.mnt->mnt_flags & MNT_NOSUID);/d' security/selinux/hooks.c
